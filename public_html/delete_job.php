<?php
// This page is for deleting a job record.
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($_POST['sure'] == 'Yes') { // Delete the record.
		...
		// Make the query:
		$q = "DELETE FROM jobs WHERE job_id=$id LIMIT 1";		
		...
		if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
			...
			echo '<p class="error">The job could not be deleted due to a system error.</p>'; // Public message.
			echo '<p>' . mysqli_error($dbc) . '<br />Query: ' . $q . '</p>'; // Debugging message.
		}
	
	} else { // No confirmation of deletion.
		echo '<p>The job has NOT been deleted.</p>';	
	}

} else { // Show the form.

	// Retrieve the job's information:
	$q = "SELECT job_num, job_name FROM jobs WHERE job_id=$id";
	$r = @mysqli_query ($dbc, $q);
	...
	if (mysqli_num_rows($r) == 1) { // Valid job ID, show the form.
		...
		echo "<h3>Job No: $row[0]</h3><h3>Job Name: $row[1]</h3>
